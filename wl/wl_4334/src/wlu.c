 ...
 *
 * $Id: wlu.c 325919 2012-04-05 03:26:23Z $
 */
...
static void wl_txpwr_array_print(int8 *powers, int mimo, bool bandwidth_is_20MHz, bool verbose);
 ...
 */
#define CURPOWER_OUTPUT_FORMAT_VERSION "3.2"
...
wl_get_current_power(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* curpower will crash in mpc = 1 mode. */
	if (wlu_iovar_getint(wl, "mpc", &mpc) < 0)
		mpc = 1;  /* error retreiving mpc - assume the worst */
	if (mpc) {
		fprintf(stderr, "Error: must be in mpc 0 mode to use curpower\n");
		return IOCTL_ERROR;
	}
	...
	printf("Board Limits:\n");
	wl_txpwr_array_print(power.board_limit, mimo, power.bandwidth_is_20MHz, verbose);
	printf("\n");

	printf("Power Target:\n");
	wl_txpwr_array_print(power.target, mimo, power.bandwidth_is_20MHz, verbose);
	printf("\n");
...
static void
wl_txpwr_array_print(int8 *powers, int mimo, bool bandwidth_is_20MHz, bool verbose)
{
	int i, j;
...
wl_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	void *ptr;
	uint16 ver;
	FILE* file = fopen("/data/misc/wifi/tmp.txt", "w+");
...
#ifdef ANDROIDTEST
	//allcorrectPkt = dtoh32(cnt.rxdfrmucastmbss);
	//allcorrectPkt = dtoh32(cnt->rxdfrmocast);
	allcorrectPkt = dtoh32(cnt->pktengrxducast);
#endif
